...
class _TambahKomponenState extends State<TambahKomponen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.komponen == null ? 'Tambah Komponen Penilaian' : 'Edit Komponen Penilaian'),
      ),
      body: Padding(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                ...
                decoration: InputDecoration(labelText: 'Judul Komponen Penilaian'),
                validator: (value) {
  ...
  void _submitForm() async {
    Map<String, String> komponenData = {
    ...
    if (widget.komponen == null) {
      ...
      apiUrl = 'http://127.0.0.1:8000/api/komponen/update/${widget.komponen!['id']}';
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        Navigator.of(context).pushReplacementNamed('/komponen_penilaian');
      } else {
  ...
  @override
  void dispose() {
    _namaKomponenController.dispose();
    super.dispose();
  }
}
